...
io.sockets.on("connection", function (socket) {
	// First connection, initialize a some rooms.
	if(connectionCount === 0){
		var room1 = new Room('Global', 0, 0);
		rooms[0] = room1;
		chatHistory[room1.name] = [];

		var room2 = new Room('Random Chat', 1, 1);
		rooms[1] = room2;
		chatHistory[room2.name] = [];

		var room3 = new Room('Random Chat 2', 2, 2);
		rooms[2] = room3;
		chatHistory[room3.name] = [];
		connectionCount++;
		...
		if (exists) {//provide unique username:
			...
			for (var room in rooms ) {
  				if (rooms.hasOwnProperty(room)) {
  					console.log(rooms[room].messagesUnread)
    				rooms[room]["messagesUnread"][socket.id] = rooms[room].totalMessages;
  				}
			}

			io.sockets.emit("update-people", {people: people, count: sizePeople});
			socket.emit("roomList", {rooms: rooms, count: sizeRooms, id: socket.id });
			socket.emit("joined");
			sockets.push(socket);
		}
	});
	...
	socket.on("send", function(msTime, msg) {
			...
			if (io.sockets.manager.roomClients[socket.id]['/'+socket.room] !== undefined ) {
				...
				for (var room in rooms ) {
					...
					console.log(room)
  					if (rooms.hasOwnProperty(room)) {
  						if(rooms[room].name === socket.room){
  							for(var socketID in rooms[room]["messagesUnread"] ){
  								if (rooms[room]["messagesUnread"].hasOwnProperty(socketID)) {
  									rooms[room]["messagesUnread"][socketID] +=1;
  								}
  							}
  						}
  					}
				...
				io.sockets.emit("roomList", {rooms: rooms, count: sizeRooms, id: socket.id});
	...
	socket.on("checkRoomName", function(name, fn) {
		var match = false;
	...
	socket.on("joinRoom", function(id) {
		if (typeof people[socket.id] !== "undefined") {
			var room = rooms[id];
			if (socket.id === room.owner) {
				socket.emit("update", "You created this room, you are already joined.");
			} else {
